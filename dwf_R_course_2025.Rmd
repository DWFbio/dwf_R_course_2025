...
Here's a picture displaying the code of the shapes.

![R shape codes](images/pch-symbols.png)
...
* making the x-axis label large and bold
* making the y-axis label italic
* making the "tick labels" larger
* centering the title, and making it large and bold
* making the subtitle larger and italic
...
ggplot(results_long, aes(x = Sample_ID, y = Proportion, fill = Cluster)) +
  ...
  theme_minimal() +
  labs(x = "Sample", y = "Assignment Proportion", fill = "Cluster") +
  theme(legend.position = "none",
        axis.text.x = element_text(angle = 45, hjust = 1))
```
...
The second critical reason for assuming neutral evolution - that is, the presence of genetic drift and mutation - is that the presence of **population structure itself violates HWE**: population subdivision violates the assumption of random mating. This is compounded by restricted gene flow between populations, resulting in genetic drift acting independently in each population.
